...
class InvoiceEntryTest extends Specification {
    ...
    def "When equal() called on same objects should return true"() {
        given:
        def entry = TestHelper.createInvoices()[0].getEntries()
    ...
    def "When equal() called on different objects should return false"() {
        given:
        def entry1 = TestHelper.createInvoices()[0].getEntries()
        def entry2 = TestHelper.createInvoices()[1].getEntries()
        def entry3 = TestHelper.createInvoices()[2].getEntries()
    ...
    def "setDescription called should modify description"() {
        given:
        def entry = TestHelper.createInvoices()[0].getEntries()[0]
    ...
    def "setPrice called should modify price"() {
        given:
        def entry = TestHelper.createInvoices()[1].getEntries()[0]
    ...
    def "setVatRate called should modify Vat rate"() {
        given:
        def entry = TestHelper.createInvoices()[2].getEntries()[0]
